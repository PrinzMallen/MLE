package Michael.HillClimbingTwo;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < anzahlDerDurchlaeufe; i++) {
            ...
            float temperatur = (float)anzahlDerDurchlaeufe/((float)i + 0.1f);
            System.out.println(temperatur);
            double wahrscheinlichkeit = erstelleWahrscheinlichkeit(aktuelleDistanz, neueDistanz, temperatur)*1000;
            
            if (aktuelleDistanz >= neueDistanz || ((erstelleZufallsInt(0, 1000)) < wahrscheinlichkeit)){
            	System.out.println("Distanzminderung in Schritt "+ i + ": von " + aktuelleDistanz + " auf " + neueDistanz + " gesunken. Wahrscheinlichkeit: " + wahrscheinlichkeit);
            }else{
	...
	public static double erstelleWahrscheinlichkeit(int aktuelleDistanz, int neueDistanz, double temperatur)
    {
        if (aktuelleDistanz >= neueDistanz){
        	return 1;
        }
        return Math.exp(((double)aktuelleDistanz - (double)neueDistanz) / temperatur);
    }
}
